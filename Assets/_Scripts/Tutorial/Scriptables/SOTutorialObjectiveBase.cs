...
public abstract class SOTutorialObjectiveBase : ScriptableObject
{
    ...
    protected void UpdateText(TutorialManager manager)
    {
        ...
        if (currentPageData.HasTasks())
        {
            foreach (ObjectiveCondition currentCondition in currentPageData.SO_Tasks)
            {
    ...
    {
        foreach (ObjectiveCondition condition in _runtimeTutorialData.SO_Pages[_currentPage].SO_Tasks)
        {
    ...
    public void CloneData()
    {
        ...
        for (int i = 0; i < SO_tutorialData.SO_Pages.Count; i++)
        {
            _runtimeTutorialData.SO_Pages[i] = Instantiate(SO_tutorialData.SO_Pages[i]);

            for (int task = 0; task < SO_tutorialData.SO_Pages[i].SO_Tasks.Count; task++)
            {
                _runtimeTutorialData.SO_Pages[i].SO_Tasks[task] = Instantiate(SO_tutorialData.SO_Pages[i].SO_Tasks[task]);
            }
        }
    }
}
